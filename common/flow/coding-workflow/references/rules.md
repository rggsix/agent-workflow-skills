...
5. Project review（**启动 subagent** 调用 `project-review`；基于 `project` skill 对本次编码结果做项目规范 review）
6. AI review（自检闭环）
7. 项目记忆收尾（调用 `project-memory`，把当前状态、关键决策与最近进展写回持久记忆）
...
5. 步骤5 Project review -> `project-review`
    - **‼️必须启动 subagent 执行该 skill!!!**
6. 步骤6 AI review -> `ai-review`
7. 步骤7 项目记忆收尾 -> `project-memory`
...
- 步骤5必须检查是否存在 `project` 相关 skill；若无，输出"无相关skill, 跳过此步骤, 流程继续进行"，然后继续步骤6。
- 步骤7是 workflow 默认收尾步骤；完成步骤6后不得直接宣告结束，需先完成 `project-memory` 写回。
